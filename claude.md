...
- **QR CDN : deux bugs successifs, même symptôme.** (1) `/build/qrcode.min.js` → 404, cassé dès le premier commit. (2) Le fix suivant, `/lib/browser.min.js`, répond 200 mais contient `require("./core/qrcode")` — CommonJS, inexistant en navigateur, plante silencieusement à l'exécution (`QRCode` ne se définit jamais malgré le 200 OK). **Un `curl` qui vérifie juste le status HTTP ou grep un nom de symbole ne prouve rien** : il faut que le code s'exécute réellement. Fix final vérifié par exécution réelle (import ESM Node, dépendances résolues à la main, `create()` appelé pour de vrai) : `<script type="module">import QRCode from 'https://cdn.jsdelivr.net/npm/qrcode@1.5.3/+esm'; window.QRCode = QRCode;</script>` — jsdelivr `+esm` bundle et résout les dépendances internes en un vrai module ES exécutable, contrairement au fichier `lib/` brut. C'est un module (chargement différé), donc la lib peut ne pas être encore prête si `renderQrInto()` est appelé extrêmement tôt — géré par le fallback déjà en place (`!('QRCode' in window)`), pas un souci en pratique (le module charge bien avant qu'un humain ait le temps de cliquer).
